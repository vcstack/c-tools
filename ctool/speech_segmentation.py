...
from .align_languages import EXTRA_ALIGN, INVERTED_LANGUAGES
from .logging_setup import logger
from .media_utils import remove_directory_contents, run_command, sanitize_file_name
    ...
    if max(min_speakers, max_speakers) > 1 and model_name:
        ...
        for segment in result_diarize["segments"]:
            if "speaker" not in segment:
                ...
                logger.warning(
                    f"No speaker detected at {segment['start']}; "
                    f"defaulting to SPEAKER_00 for: {segment['text'][:80]!r}"
                )
